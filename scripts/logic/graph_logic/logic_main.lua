...
deltarune_location = {}
deltarune_location.__index = deltarune_location
...
function deltarune_location.new(name)
    local self = setmetatable({}, deltarune_location)
    if name then
...
function deltarune_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = deltarune_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function deltarune_location:connect_two_ways(exit, rule)
...
function deltarune_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function deltarune_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function deltarune_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function deltarune_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function deltarune_location:accessibility()
...
-- 
function deltarune_location:discover(accessibility, keys)
...
entry_point = deltarune_location.new("entry_point")
